...
class DefaultRepositoryCollector {
    ...
    DefaultRepositoryCollector(Collection<Class<? extends Repository>> defaultRepositoryImplementations, Application application) {
    ...
    Collection<BindingStrategy> collect(Collection<Class<? extends AggregateRoot>> aggregateClasses) {
        ...
        for (Class<?> aggregateClass : BusinessUtils.includeSuperClasses(aggregateClasses)) {
            ...
            Key<?> defaultKey = BusinessUtils.defaultQualifier(application, DEFAULT_REPOSITORY_KEY, aggregateClass, genericInterface);
